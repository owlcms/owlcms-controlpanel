#!/usr/bin/env bash
# Sign, notarize, and staple the stable Automator installer application locally.

set -euo pipefail

usage() {
	cat <<'EOF'
Usage: scripts/automator/notarize-automator-installer-app.sh
...
app="$script_dir/Install controlpanel.app"
...
workspace=$(mktemp -d "${TMPDIR:-/tmp}/owlcms-automator-app-notarization.XXXXXX")

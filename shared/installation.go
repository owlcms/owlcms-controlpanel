...
func DetectInstalledModules(owlcmsPath, trackerPath, firmataPath, videoPath string) ModulePresence {
	owlexists := dirExists(owlcmsPath)
	trackerexists := dirExists(trackerPath)
	firmataexists := dirExists(firmataPath)
	videoexists := dirExists(videoPath)
	...
	log.Printf("DetectInstalledModules: checked Video path=%s exists=%v", videoPath, videoexists)

	...
	fileMenu := fyne.NewMenu("File",
		...
		fyne.NewMenuItem("Install version from ZIP", func() {
			...
			selectLocalZip(w, func(path string, err error) {
				...
				installUtils.ProcessLocalZipFile(path, w, owlcmsInstallDir, copyFile, updateExplanation, recomputeVersionList, checkForNewerVersion)
			})
		}),
		fyne.NewMenuItem("Save installed version as ZIP", func() {
			installUtils.ZipCurrentSetup(w, owlcmsInstallDir, getAllInstalledVersions, selectSaveZip)
		}),
	)

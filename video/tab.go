...
func CreateTab(w fyne.Window) *fyne.Container {
	...
	replaysColumn := container.NewVBox(
		widget.NewLabelWithStyle("Replays", fyne.TextAlignLeading, fyne.TextStyle{Bold: true}),
		replaysStopButton,
		replaysDirLink,
		replaysLogLink,
	)
	...
	stopContainer = container.NewVBox(
		widget.NewSeparator(),
		container.NewGridWithColumns(2, camerasColumn, replaysColumn),
		statusLabel,
	)
	...
	replaysStopButton.OnTapped = func() {
		dialog.NewConfirm("Confirm Stop", "Stop the running Replays process?",
			func(confirm bool) {
				if confirm {
					stopReplaysProcess(replaysProcess, replaysVersion, replaysStopButton, w)
				}
